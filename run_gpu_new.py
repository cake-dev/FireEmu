...
def run_simulation(params, run_id, output_dir):
    ...
    history_moisture = np.zeros((num_frames, nx, ny, nz), dtype=np.float16)
    ...
    for t in range(total_steps):
        ...
        if t % save_interval == 0 and frame_idx < num_frames:
            ...
            history_moisture[frame_idx] = fuel_moisture_dev.copy_to_host().astype(np.float16)
